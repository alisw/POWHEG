...
GOSAMLIBDIR=$RUNDIR/GoSamlib
VIRDIR=$GOSAMDIR/Virtual
CONTRIBDIR=$GOSAMSTUFFDIR/GoSam-contrib-2.0
CONTRIBDIRNINJA=$CONTRIBDIR/ninja-1.0.0
...
then
	...
	read -p "Would you like to give the path to the orderfile by hand? [Y/n]: " REPLY
	if [ $REPLY = "Y" ]
	then
	    read -p "Type path to orderfile from process folder: " ORDERPATH
	    if [ -f $RUNDIR/$ORDERPATH ]
	    then
		cp $RUNDIR/$ORDERPATH $GOSAMDIR/orderfile.lh
	    else
		echo "Path incorrect, STOP!"
		exit
	    fi
	else
	    echo "STOPPED!"
...
then
	...
	# Ninja files:
	mkdir -p $GOSAMLIBDIR/ninja
	\cp $CONTRIBDIRNINJA/ninja/*.hh $GOSAMLIBDIR/ninja
	\cp $CONTRIBDIRNINJA/ninja/ninja_config.h $GOSAMLIBDIR/ninja
	\cp $CONTRIBDIRNINJA/config.h $GOSAMLIBDIR
	\cp $CONTRIBDIRNINJA/*.hh $GOSAMLIBDIR/
	\cp $CONTRIBDIRNINJA/*.F90 $GOSAMLIBDIR/
	\cp $CONTRIBDIRNINJA/*.cxx $GOSAMLIBDIR/
	\cp $CONTRIBDIRNINJA/*.cc $GOSAMLIBDIR/

        # create the dependencies file
	cd $GOSAMLIBDIR
	
	makedepf90 *.f90 *.F90 > deps.txt
	...
	chmod +x $GOSAMLIBDIR/compile_gosamlib.sh
