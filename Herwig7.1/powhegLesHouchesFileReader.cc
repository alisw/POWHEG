...
//vector<string> powhegLesHouchesFileReader::optWeightNamesFunc() { return optionalWeightsNames; }
...
bool powhegLesHouchesFileReader::doReadEvent() {
  ...
  if(repeatEvent){
    ...
    // copy it back here.
    hepeup=hepeup_save;
    eventComments=eventComments_save;
    outsideBlock=outsideBlock_save;
    optionalWeights=optionalWeights_save;
    optionalWeightsTemp=optionalWeightsTemp_save;
    eventAttributes=eventAttributes_save;
    optionalnpLO=optionalnpLO_save;
    optionalnpNLO=optionalnpNLO_save;
    showerSuccess=false;
    //std::cout<<"vetoing event ..."<<endl;
  ...
  // Read any additional comments and named weights.

  //  bool readingWeights = false, readingaMCFast = false, readingMG5ClusInfo = false;
  while ( cfile.readline() && !cfile.find("</event>")) {
    string line="";
    cfile >> line;
    if(line.compare("#rwgt")==0){
      cfile>>weights_.radtype;
    }
      
    if(line.compare("<weights>")==0)
      {
	int i=0;
	while ( cfile.readline() && !cfile.find("</weights>"))
	  {
	    cfile >> weights_.weight[i];
	    i++;
	    if(i > (max_num_weights-1))
	      {
		std::cout<<" Number of weights > "<< max_num_weights<<endl;
		std::cout<<" Increase max_num_weights in powhegLesHouchesFileReader.cc" <<endl;
		
		exit(-1);
	      }
	  }
	weights_.numweights=i;
      }
  }


  // //THIS IS THE DEFAULT IMPLEMENTATION, NOT COMPATIBLE WITH OUR EVENT FILES
  // bool readingWeights = false, readingaMCFast = false, readingMG5ClusInfo = false;
  // while ( cfile.readline() && !cfile.find("</event>")) {
  ...
  hepeup_save=hepeup;
  eventComments_save=eventComments;
  outsideBlock_save=outsideBlock;
  optionalWeights_save=optionalWeights;
  optionalWeightsTemp_save=optionalWeightsTemp;
  eventAttributes_save=eventAttributes;
  optionalnpLO_save=optionalnpLO;
  optionalnpNLO_save=optionalnpNLO;

  /*
   showerSuccess is false until Herwig manages to finish an event.
   If the event reaches the analysis than it will be set to true, otherwise
   it remains false. Herwig has a maximum number of tries (maxShowerTries)
   to shower an event (NB: These are tries for the shower to succeed, NOT to pass the veto)
  */ 
...
powhegLesHouchesFileReader::initpowhegLesHouchesFileReader;
...
void powhegLesHouchesFileReader::Init() {
  ...
  static Parameter<powhegLesHouchesFileReader,string> interfaceFileName
    ("FileName",
     ...
     &powhegLesHouchesFileReader::theFileName, "", false, false);
  ...
  static Switch<powhegLesHouchesFileReader,bool> interfaceQNumbers
    ("QNumbers",
     ...
     &powhegLesHouchesFileReader::theQNumbers, false, false, false);
  ...
  static Switch<powhegLesHouchesFileReader,bool> interfaceIncludeFxFxTags
    ("IncludeFxFxTags",
     "Include FxFx tags",
     &powhegLesHouchesFileReader::theIncludeFxFxTags, false, true, false);
  ...
  static Switch<powhegLesHouchesFileReader,bool> interfaceIncludeCentral
    ("IncludeCentral",
     ...
     &powhegLesHouchesFileReader::theIncludeCentral, false, true, false);
  ...
  static Reference<powhegLesHouchesFileReader,Decayer> interfaceDecayer
    ("Decayer",
     ...
     &powhegLesHouchesFileReader::theDecayer, false, false, true, true, false);
