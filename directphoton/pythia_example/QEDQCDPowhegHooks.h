/*
  This is a modified copy of PowhegHooks.h by R. Corke 
  and is supposed
  to be able to differentiate between two differing scales
  for QED and QCD radiation. This is done to match the 
  shower to my POWHEG Direct Photon code.
  For this the doVetoMPIStep function which sets the hard 
  scale of every event and the doVetoISR/FSREmission
  functions had to be edited in some lines.
  Only pThardMode = 0 is affected by the changes!
*/

#ifndef Pythia0_QEDQCDPowhegHooks_H
#define Pythia0_QEDQCDPowhegHooks_H
...
class QEDQCDPowhegHooks : public UserHooks {

public:

  // Constructor and destructor.
   QEDQCDPowhegHooks() : scalesAtt("uborns") {}
  ~QEDQCDPowhegHooks() {}
...
//--------------------------------------------------------------------------
  ...
  inline int  numberVetoMPIStep() { return 1; }
    inline bool doVetoMPIStep(int nMPI, const Event &e) {
      ...
      bool hasPhoton = false; // Photon present in final state of hard event
      int    count = 0;
	...
	exit(1);
      ...
      // If there is no radiation or if pThardMode is 0 then find out if
      // underlying Born is QCD or QED and 
      if (!isEmt) {
	if (hasPhoton) { // QCD emission was evolved down to scalup
	  pThardQCD = infoPtr->scalup();
	  pThardQED = 0.5 * infoPtr->getScalesAttribute(scalesAtt);
	}
	else { // QED emission was evolved down to scalup
	  pThardQED = infoPtr->scalup();
	  pThardQCD = 0.5 * infoPtr->getScalesAttribute(scalesAtt);	  
	}

	// if there was a POWHEG photon emission, then only QED radiation was
	// evolved to scalup, QCD should start at scale of underlying Born
      } else if (pThardMode == 0) {
	if (e[iEmt].id() == 22) {
	  pThardQED = infoPtr->scalup();
	  pThardQCD = 0.5 * infoPtr->getScalesAttribute(scalesAtt);
	}
	else { // if QCD radiation, it is the other way round
	  pThardQCD = infoPtr->scalup();
	  pThardQED = 0.5 * infoPtr->getScalesAttribute(scalesAtt);	  
	}
...
//--------------------------------------------------------------------------
  ...
  inline bool doVetoISREmission(int, const Event &e, int iSys) {
    ...
    // Set hard matching scale according to type of emission
    double pThard = (e[iEmt].id() == 22) ? pThardQED : pThardQCD;
...
private:
  ...
  double pThardQCD, pThardQED, pTMPI;
  bool   accepted, isEmt;
