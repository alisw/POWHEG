c     Taken from dijet (arXiv:1012.3380)
...
C -      call checkmass(2,kn_pborn(0,3))
      ...
      include 'pwhg_kn.h'
      include 'pwhg_math.h'
      logical ini, thetasupp
      data ini,thetasupp/.true.,.false./
      real*8 fact,pt2,pt2supp,powheginput,pt,theta
      save ini,pt2supp,pt,theta,thetasupp
      if (ini) then
         ...
         theta = powheginput("#theta")
         if (theta.gt.0) then
            thetasupp = .true.
            write(*,*) 'Additional suppression of low pT activated.'
         endif
         ini = .false.
         pt2supp = pt**2
      endif
      if (pt.le.0) then
         ...
         if (thetasupp) then
            fact = fact * (DATAN((DSQRT(pt2)-theta)*1.d4) + pi/2.d0)/pi
         endif
      endif
      end
      ...
      include 'nlegborn.h'
      include 'pwhg_kn.h'
c     For now also set em_muren2 in here
      include 'pwhg_em.h'
c
      ...
      if(.not.fixedscale) then
         muref=sqrt(kn_cmpborn(1,3)**2+kn_cmpborn(2,3)**2) ! photon pt
         if (muref.LT.1.5d0) then
            muref = 1.5d0 ! should suffice for PDFs to work
         endif
      endif
      muf=muref
      mur=muref
