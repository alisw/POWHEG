      ...
      real * 8 cqed(-6:6),gammaqed(-6:6),gammapqed(-6:6)
      save cqed,gammaqed,gammapqed
      ...
      if(ini) then
         do j=-6,6
            if(j.eq.0) then
               ...
               gammatilde(j)=cf/2
               gammaqed(j)= 3.d0/2.d0 * chargeofparticle(j)**2
               gammapqed(j)= (13.d0/2.d0-2.d0*pi**2/3.d0) *
     &              chargeofparticle(j)**2
               cqed(j) = chargeofparticle(j)**2
            endif
         enddo
         ini=.false.
      endif
...
c WEW
               if (flg_with_em.and.chargeofparticle(fl).ne.0) then
                  if(kn_masses(leg).eq.0) then
c     eq. (5.6) from FKS adopted for QED, i.e. with gamma(i) = 3/2 Q_i^2
c     and gammap(i) = (13/2-2*Pi^2/3) Q_i^2, c(i) = Q_i^2
                     Qem = Qem + gammapqed(fl)
     1                    - log(s/em_muren2) * (gammaqed(fl) - 
     2                    2 * cqed(fl) * log(2*e/(par_csicut*etot)))
     3                    + 2 * cqed(fl) * (log(2*e/etot)**2
     4                    - log(par_csicut)**2)
     5                    - 2 * gammaqed(fl) * log(2*e/etot)
                  else
                     Qem = Qem - 
...
c     set Q,Qem,I,Iem to zero if no QCD or QED radiation allowed
c     TODO: if-clauses somewhere above to prevent actual calculation of Q,I
            if (.NOT.flst_qcdqedrad(jb,1)) then ! no QCD radiation
               Q = 0.d0
               I = 0.d0
            endif
            if (.NOT.flst_qcdqedrad(jb,2)) then ! no QED radiation
               Qem = 0.d0
               Iem = 0.d0
            endif
