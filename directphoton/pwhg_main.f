     ...
     1     num_weights,num_old_weights,count_weights
      real * 8, allocatable :: weights(:,:), sudaweights(:), ubornss(:)
      logical exist
...
c this is needed for fullrwgt.
         ...
         if (flg_sudakov_rwgt .and. num_weights < 1) then
            write(*,*) "When running with 
     1        the sudakov weight has not been properly stored during
     2        the first run. When using the enhancedradfac feature
     3        make sure to request at least one rwl weights. For 
     4        more details see the example in testrun-lhc, for 
     5        questions contact Ana Cueto, Stefan Richter and Tomas 
     6        Jezo."
            call exit(-1)
         endif
         num_old_weights = 0
...
c allocate a weight array of the appropriate size
     ...
     1' Since noevents is specified, testplots will be produced'
      ...
      if(flg_rwl) then
         ...
         if (flg_sudakov_rwgt)
     1      allocate(sudaweights(num_stored_evt))
         allocate(ubornss(num_stored_evt))
      else
         num_stored_evt = 0
         num_weights = 0
         count_stored_evt = 0
...
c     read num_stored_evt from input .lhe file
     ...
     1              rwl_weights(1:num_old_weights)
               if (flg_sudakov_rwgt) 
     1            sudaweights(count_stored_evt) = 
     2               rwl_weights(1)/xwgtup
                  weights(1:num_old_weights,count_stored_evt) =
     1               rwl_weights(1:num_old_weights)/
     2               sudaweights(count_stored_evt)
                  ubornss(count_stored_evt) = uborns
            elseif (flg_sudakov_rwgt) then
               write(*,*) "When using the enhancedradfac feature
     1           make sure to request at least one rwl weights. 
     2           Cannot proceed! For more details see the example 
     3           in testrun-lhc, for questions contact Ana Cueto, 
     4           Stefan Richter and Tomas Jezo."
               call exit(-1)
            endif
...
c     weight is retrieved from the event, saving time ...
     ...
     1                 (weights(count_weights,k_stored_evt))
                  if (.not. flg_rwl_add) then
                     if (flg_sudakov_rwgt) 
     1                  sudaweights(k_stored_evt) = sudawgt
                     ubornss(k_stored_evt) = uborns
                  endif
               enddo
...
c     Write event and weights
               ...
               sudawgt = sudaweights(k_stored_evt)
               uborns = ubornss(k_stored_evt)
               call lhefwritev(iunout)
