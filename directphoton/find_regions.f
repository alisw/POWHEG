...
c     keep information on uborns being admissible for QED or QCD
c     radiation
      flst_qcdqedrad = .FALSE.
      do j = 1, flst_nborn      ! just check if born has FS photon
         do k = 3, nlegborn
            if (flst_born(k,j).eq.22) then
               flst_qcdqedrad(j,1) = .TRUE.
            endif
         enddo
         if (.NOT.flst_qcdqedrad(j,1)) then
            flst_qcdqedrad(j,2) = .TRUE.
         endif
      enddo
      end
