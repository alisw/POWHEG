...
c NR06:   d sigma = ppsv * f(x1) * f(x2) * gs^6/(16 pi 16 pi^2)   beta dcosth1 dx1 dx2
c NR06:                                         as^3/4
      function ppsv(s,t,xm2,xmur2)
...
c         write(*,*) 'aa:',qqqq1(s,t,xm2,xmur2,nl)/qqqq(s,t,xm2,xmur2,nl)
      ...
      vlwm = dlog(aw/m2)
      vlbl = dlog(b/lm)
      vdw = ddilog((aw-m2)/aw)-vlwm**2/2.0d+0
      vdt = ddilog((at-m2)/at)-vltm**2/2.0d+0
      vdmp = ddilog(-lm/lp)
      vdmb = vlbl**2/2.0d+0+ddilog(-lm/b)
      auinv = 1/(m2-aw)
      atinv = 1/(m2-at)
     ...
     3   1.6d+1*pi**2*ro**2/9.0d+0)/(s*(t1-1)**3*t1**3)/1.92d+2
      ggqq = (ss+dd)/pi/4.0d+0
      return
      end

      function qqqq(s,t,m2,mu2)
      implicit double precision (a-z)
      integer nl
      common/nl/nl
      data pi/3.141 592 653 589 793/
      ...
      vlwm = dlog(aw/m2)
      vlbl = dlog(b/lm)
      vdw = ddilog((aw-m2)/aw)-vlwm**2/2.d+0
      vdt = ddilog((at-m2)/at)-vltm**2/2.d+0
      vdmp = ddilog(-lm/lp)
      vdmb = vlbl**2/2.d+0+ddilog(-lm/b)
      auinv = 1/(m2-aw)
      atinv = 1/(m2-at)
...
c**********************
     ...
     5   .8d+1)*t1/3.0d+0+(-2.0d+0)*ro**3/3.0d+0+4.0d+0*ro**2/3.0d+0)*vl
     6   pm/(b*s*(t1-1)**2*t1**2)/9.6d+1
      dd = dd-(2*nl-33)*pi*(3*t1**2-3*t1+4.0d+0/3.0d+0)*(8*t1**4-16*t1**
     1   3+4*ro*t1**2+12*t1**2-4*ro*t1-4*t1+ro**2)*vlm2/(s*(t1-1)**2*t1*
     2   *2)/2.4d+1
      dd = dd-auinv*pi*(3*t1+(-1.0d+0)/3.0d+0)*(4*t1+ro-4)*((-8.0d+0)*t1
     1   **4/3.0d+0+1.6d+1*t1**3/3.0d+0-4*ro*t1**2+2.8d+1*t1**2/3.0d+0+4
     ...
     2   *6+640*pi**2*ro*t1**6+432*nl*ro*t1**6-14688*ro*t1**6+2.9764d+4*
     3   pi**2*t1**6/3.0d+0+7.424d+3*nl*t1**6/3.0d+0-91072*t1**6+108*pi*
     4   *2*ro**2*t1**5-864*ro**2*t1**5-1920*pi**2*ro*t1**5-1296*nl*ro*t
     5   1**5+44064*ro*t1**5-9604*pi**2*t1**5-2048*nl*t1**5+71616*t1**5+
     6   4.12d+2*pi**2*ro**2*t1**4/9.0d+0+96*nl*ro**2*t1**4-2712*ro**2*t
     7   1**4+6.754d+3*pi**2*ro*t1**4/3.0d+0+4.436d+3*nl*ro*t1**4/3.0d+0
     8   -50496*ro*t1**4+5.002d+4*pi**2*t1**4/9.0d+0+896*nl*t1**4+(-8.45
     9   12d+4)*t1**4/3.0d+0+(-2.444d+3)*pi**2*ro**2*t1**3/9.0d+0-192*nl
     :   *ro**2*t1**3+6864*ro**2*t1**3+(-3.908d+3)*pi**2*ro*t1**3/3.0d+0
     ;   +(-2.392d+3)*nl*ro*t1**3/3.0d+0+27552*ro*t1**3+(-1.646d+4)*pi**
     <   2*t1**3/9.0d+0+(-5.12d+2)*nl*t1**3/3.0d+0+1.2544d+4*t1**3/3.0d+
     =   0+7.0d+2*pi**2*ro**2*t1**2/3.0d+0+4.16d+2*nl*ro**2*t1**2/3.0d+0
     >   +(-1.5592d+4)*ro**2*t1**2/3.0d+0+342*pi**2*ro*t1**2+5.48d+2*nl*
     ?   ro*t1**2/3.0d+0+(-1.9168d+4)*ro*t1**2/3.0d+0+2.464d+3*pi**2*t1*
     @   *2/9.0d+0+7.36d+2*t1**2/3.0d+0+(-7.16d+2)*pi**2*ro**2*t1/9.0d+0
     1   +(-1.28d+2)*nl*ro**2*t1/3.0d+0+4.864d+3*ro**2*t1/3.0d+0+(-3.2d+
     ...
     2   nlf*ro-496*ro-20*pi**2+160*nlf-2384)*zg**6/s/8.1d+1
      qqqq2 = ss+dd/pi/4.0d+0
      return
      end


      function ggqq2(s,t,m2,mu2)
      implicit double precision (a-z)
      integer nl
      character * 2 scheme
      common/nl/nl
