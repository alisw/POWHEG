...
c     sets born momenta in kin. common block
         ...
         suppfact=1
c set scales
c         call setscalesbtilde
c         call allborn
c     sets xscaled, y, phi in kinematics common block
c     call btildeborn(resborn)
         resborn=0
         resvirt=0
         resreal=0
         rescoll=0

         resborn(1) = xborn(1)**(-0.2d0)/3.8937966d8
         resborn(2) = -resborn(1)/2
         if (.not.flg_bornonly.and..not.imode.eq.0) then
            resvirt(1)=xborn(1)**(-0.25d0)/3.8937966d8
            resvirt(2)=-resvirt(1)/2
         endif
c     accumulate values
         retval=0
         do j=1,flst_nborn
